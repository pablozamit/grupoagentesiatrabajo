...
const normalize = (value) => value.normalize("NFD").replace(/\p{Diacritic}/gu, "").toLocaleLowerCase("es").replace(/[^a-z0-9]+/g, " ").trim();
const STOP_WORDS = new Set(["a", "al", "de", "del", "el", "en", "la", "las", "los", "para", "por", "que", "una", "un", "y"]);
const words = (value) => normalize(value).split(/\s+/).filter(word => word && !STOP_WORDS.has(word));
...
function fuzzyMatch(query, candidate) {
    ...
    if (candidate.startsWith(query) || query.startsWith(candidate))
        return 0.9;
    if (query.length >= 5 && candidate.length >= 5 && query.slice(0, 5) === candidate.slice(0, 5))
        return 0.84;
...
function makeSnippet(content, terms, max = 260) {
...
export class EncyclopediaIndex {
    ...
    entries = [];
    loadedAt;
    constructor(indexUrl = DEFAULT_INDEX_URL, siteUrl = DEFAULT_SITE_URL) {
    ...
    async ensureLoaded() { if (!this.entries.length)
        await this.refresh(); }
    search(query, options = {}) {
        ...
        return this.entries.flatMap(entry => {
            ...
            if (requestedTag && !tags.some(tag => normalize(tag) === requestedTag))
                return [];
            ...
            if (!score || matchedTerms < minimumMatches)
                return [];
